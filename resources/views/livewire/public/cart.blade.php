    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 py-8 md:py-12">
        ...
        @if(count($cartItems) == 0)
            <div class="text-center py-16">
                ...
                <a href="/shop" class="bg-brand-600 text-white px-6 py-3 rounded-full hover:bg-brand-700 transition-all font-medium">
                    Continue Shopping
                </a>
            </div>
        @else
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
                                    ...
                                    <div class="col-span-1 md:col-span-6">
                                        <div class="flex items-center">
                                            ...
                                            <div class="md:hidden">
                                                <button type="button" wire:click="removeItem({{ $item['id'] }})" class="text-gray-400 hover:text-brand-600 transition-all">
                                            ...
                                            <!-- Product Details -->
                                            <div class="ml-4">
                                                <h3 class="font-poppins font-medium">{{ $item['name'] }}</h3>
                                                <div class="text-sm text-gray-500">{{ $item['category'] }} • {{ $item['weight'] }}</div>
                                                <!-- Price (Mobile Only) -->
                                                <div class="mt-1 md:hidden">
                                                    <span class="text-brand-600 font-semibold">₹{{ $item['price'] }}</span>
                                                    @if($item['originalPrice'] > $item['price'])
                                                        <span class="text-gray-400 text-xs line-through ml-1">₹{{ $item['originalPrice'] }}</span>
                                                    @endif
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div class="col-span-1 md:col-span-2 md:text-center">
                                        <div class="flex items-center border border-gray-200 rounded-full max-w-[120px] mx-auto">
                                            <button type="button" 
                                                wire:click="updateQuantity({{ $item['id'] }}, {{ $item['quantity'] - 1 }})"
                                                class="w-8 h-8 flex items-center justify-center text-gray-600 hover:text-brand-600 focus:outline-none">
                                                -
                                            </button>
                                            <input type="number" min="1" value="{{ $item['quantity'] }}" 
                                                wire:change="updateQuantity({{ $item['id'] }}, $event.target.value)"
                                                class="w-10 text-center border-0 focus:ring-0 text-gray-900 text-sm focus:outline-none px-0"
                                                readOnly>
                                            <button type="button" 
                                                wire:click="updateQuantity({{ $item['id'] }}, {{ $item['quantity'] + 1 }})"
                                                class="w-8 h-8 flex items-center justify-center text-gray-600 hover:text-brand-600 focus:outline-none">
